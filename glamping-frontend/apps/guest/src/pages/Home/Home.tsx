...
type ActiveModal = ConfirmSheetType | 'food' | 'minibar' | 'transfer' | 'cleaning' | 'kupe' | 'checkout' | null
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200 pl-16">{t('home.title')}</h1>
        <ThemeToggle size="lg" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 animate-slide-up">
        ...
        <ServiceTile icon={<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 6c.6.5 1.2 1 2.5 1C7 7 7 5 9.5 5c2.6 0 2.4 2 5 2 2.5 0 2.5-2 5-2 1.3 0 1.9.5 2.5 1"/><path d="M2 12c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 2.6 0 2.4 2 5 2 2.5 0 2.5-2 5-2 1.3 0 1.9.5 2.5 1"/><path d="M2 18c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 2.6 0 2.4 2 5 2 2.5 0 2.5-2 5-2 1.3 0 1.9.5 2.5 1"/></svg>} label={t('home.kupe')} color="bg-blue-600" onClick={() => { refetchServices(); setActiveModal('kupe') }} />
        ...
        {displayCheckoutDone ? (
          <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-3xl p-5 flex flex-col justify-between h-36 transition-colors">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-500 dark:text-white/40 whitespace-pre-line">{t('checkout.requested')}</h3>
            </div>
          </div>
        ) : (
          <div onClick={() => setActiveModal('checkout')}
            ...
            <p className="text-xs text-white/70 mt-1">{t('checkout.subtitle')}</p>
          </div>
        )}
      ...
      {(activeModal === 'food' || activeModal === 'transfer' || activeModal === 'cleaning' || activeModal === 'kupe' || activeModal === 'towels') && activeModal && (
        <OrderForm
          open={true}
